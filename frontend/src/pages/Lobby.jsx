...
const Lobby = () => {
    ...
    const isHostRef = useRef(false); // ← ADD THIS
    ...
    useEffect(() => {
        ...
        const handleLobbyInfo = (data) => {
            ...
            // Only update isLobbyOwner if it's not already true (preserve host status)
            setIsLobbyOwner(prev => prev || Boolean(data.isLobbyOwner));
            isHostRef.current = Boolean(data.isLobbyOwner);
        };
        ...
        const handleGameStarted = ({ gameCode }) => {
            ...
            console.log("isHostRef.current:", isHostRef.current); // ← ADD THIS LOG
            // Navigate based on role
            if (isHostRef.current) {
                console.log('host redirecting')
                navigate(`/host/game/${gameCode}`, { replace: true }); // ADD replace: true
            } else {
                console.log('player redicerting')
                navigate(`/play/game/${gameCode}`, { 
                    replace: true, // ADD replace: true
                    state: { 
        ...
        // Join logic - run ONCE
        if (!hasJoinedRef.current && !isNavigating) {
            ...
            else if (playerNameFromState && gameCodeParam && !isNavigating) {
            console.log(
                "Player joining:",
                playerNameFromState,
                "Code:",
             ...
             isHostRef.current = false; // ← ADD THIS
            ...
            else if (user && gameCodeParam && !isNavigating) {
