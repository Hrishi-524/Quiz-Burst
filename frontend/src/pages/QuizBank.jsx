// frontend/src/pages/QuizBank.jsx
...
function QuizBank() {
  ...
  const pendingQuizId = useRef(null);
  const pendingNavigation = useRef(null); // Track which quiz is being activated
  ...
  useEffect(() => {
    const fetchQuizzes = async () => {
      ...
      const metaData = allQuizzes.quizzesMeta.map(quiz => ({
        id: quiz.id,
        title: quiz.title,
  ...
  // Set up socket listeners ONCE on mount
    useEffect(() => {
        ...
        const handleTakeGameCode = (data) => {
        console.log('takeGameCode received:', data);
        if (pendingQuizId.current) {
        ...
        socket.on('takeGameCode', handleTakeGameCode);
        socket.on('error', handleError);

        return () => {
            socket.off('takeGameCode', handleTakeGameCode);
            socket.off('error', handleError);
        };
    }, [navigate]);

  // import socket from your shared socket module
    const handleGoLive = (quizId) => {
        ...
        console.log("🚀 Going live with quiz:", quizId);

        // Ask server to create a lobby
        socket.emit('createLobby', { quizId, user });

        // Listen once — server will reply with takeGameCode or lobbyInfo
        socket.once('takeGameCode', (data) => {
            console.log('Lobby created:', data);
            // navigate and pass lobbyInfo so Lobby knows we are the creator
            navigate(`/quiz/lobby/${data.gameCode}`, {
                state: { 
                    isLobbyCreator: true, 
                    lobbyInfo: data  
        ...
        socket.on('lobbyInfo', (data) => {
            // fallback if you want to handle direct lobbyInfo reply
            console.log('lobbyInfo received on create:', data);
        });
        ...
        <div className="grid grid-cols-3 gap-3 mb-4">
          ...
          <div className="flex items-center gap-2 text-sm">
            ...
            <div>
              ...
              <div className="text-white font-semibold">{quiz.duration}</div>
            </div>
          </div>
